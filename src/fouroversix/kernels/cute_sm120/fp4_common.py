...
from cutlass import Float8E4M3FN, Float32, Int32, Int64, Uint8, Uint16, Uint32, Uint64
...
@dsl_user_op
def f32_to_u32_bits(a: Float32, *, loc=None, ip=None) -> Uint32:
    return Uint32(
        llvm.inline_asm(
            T.i32(),
...
@cute.jit
def cvt_f32_to_e4m3(a: Float32) -> Uint32:
    return Uint32(Float8E4M3FN(a).bitcast(Uint8))


@cute.jit
def cvt_e4m3_to_f32(fp8_val: Uint32) -> Float32:
    fp8_u8 = Uint8(fp8_val & cutlass.Uint32(0xFF))
    return fp8_u8.bitcast(Float8E4M3FN).to(Float32)
...
def nvfp4_compute_output_scale(
    ...
    scale_f32 = cvt_e4m3_to_f32(fp8_val)
    output_scale = Float32(0.0)
    if scale_f32 != Float32(0.0):
        decode_scale = Float32(1.0) / global_scale
        output_scale = Float32(1.0) / (decode_scale * scale_f32)
    return output_scale
...
def nvfp4_compute_dequant_scale(
    ...
    return cvt_e4m3_to_f32(fp8_val) / global_scale
...
def nvfp4_compute_dequant_scale_from_amax(
    ...
    return cvt_e4m3_to_f32(fp8_val) * amax_over_global_scale_denominator
...
def nvfp4_compute_quant_scale_exact(
    ...
    return nvfp4_compute_output_scale(fp8_val, global_scale)
...
@dsl_user_op
def bfloat2_nvfp4_mse_error_scalar_triton_dequant(
    h2: Uint32,
    output_scale: Float32,
    fp8_val: Uint32,
    amax: Float32,
...
def bfloat2x8_to_e2m3x16_packed(
    ...
    return (Uint64(p1) << Uint64(32)) | Uint64(p0), (Uint64(p3) << Uint64(32)) | Uint64(
        p2
    )
