...
from fouroversix.kernels.cute_sm120.fp4_common import (
...
def _blackwell_scale_index(
    ...
    return (
        (
            (row_group * Int32(scale_blocks_per_row // 4) + col_group) * Int32(32)
...
def _process_nvfp3_static_block_bfloat_transposed(
    ...
    if cutlass.const_expr(scale_block_size == 8):
        ...
        h4 = _ld_transposed_bfloat2(
            x, elem_base + Int32(8), elem_base + Int32(9), col_idx
        )
        h5 = _ld_transposed_bfloat2(
            x,
            elem_base + Int32(10),
...
def _process_nvint6_static_block_bfloat_transposed(
    ...
    h5 = _ld_transposed_bfloat2(
        x, elem_base + Int32(10), elem_base + Int32(11), col_idx
    )
    h6 = _ld_transposed_bfloat2(
        x, elem_base + Int32(12), elem_base + Int32(13), col_idx
    )
    h7 = _ld_transposed_bfloat2(
        x, elem_base + Int32(14), elem_base + Int32(15), col_idx
    )
...
def _process_nvfp6_static_block_bfloat(
    ...
    scale_float = scale_global_scale * (block_max / Float32(float(max_quantized_value)))
    scale_fp8_u32 = cvt_f32_to_e4m3(scale_float)
    scale_fp8 = Uint8(scale_fp8_u32 & cutlass.Uint32(0xFF))
    output_scale = nvfp4_compute_output_scale(scale_fp8_u32, quant_global_scale)
...
def _process_if3_adaptive_block_bfloat(
    ...
    block_max_h2 = bfloat2_max_abs_8(h0, h1, h2, h3, h4, h5, h6, h7)
    block_max = bfloat2_hmax_reduce_to_f32(block_max_h2)
    scale_float = scale_global_scale * (block_max / Float32(E2M0_MAX))
    scale_fp8_u32 = cvt_f32_to_e4m3(scale_float)
    scale_fp8 = Uint8(scale_fp8_u32 & cutlass.Uint32(0xFF))
    output_scale = nvfp4_compute_output_scale(scale_fp8_u32, quant_global_scale)
...
def _process_if3_adaptive_pseudo_block_bfloat(
    ...
    intq0, intq1, intq2, intq3 = _int3_quant4_bfloat_values(h0, h1, int_output_scale)
    intq4, intq5, intq6, intq7 = _int3_quant4_bfloat_values(h2, h3, int_output_scale)
    ...
    if cutlass.const_expr(scale_block_size == 8):
        ...
        fpq8, fpq9, fpq10, fpq11 = _fp3_quant4_bfloat_values(h4, h5, output_scale)
        fpq12, fpq13, fpq14, fpq15 = _fp3_quant4_bfloat_values(h6, h7, output_scale)
...
class Sm120NVFP4StaticQuantize:
    def __init__(
        self,
        k: int,
...
class Sm120NVFP4StaticTransposeQuantize:
    def __init__(
        self,
        k: int,
        max_quantized_value: int,
...
class Sm120NVFP4AdaptiveQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
        stochastic_rounding: bool = False,
        scale_block_size: int = NVFP4_SCALE_BLOCK_SIZE,
        min_blocks_per_mp: int = BLOCKS_PER_SM,
    ...
    def kernel(
        ...
        sf_idx = total_scale_blocks + bidx * NVFP4_BASE_THREADS_PER_BLOCK + tidx
        while sf_idx < allocated_scale_blocks:
            scales[sf_idx] = Uint8(0)
            sf_idx = sf_idx + stride


class Sm120NVFP4AdaptiveSmallMQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
    ...
    def kernel(
        ...
        while sf_idx < total_scale_blocks:
            ...
            if cutlass.const_expr(self.scale_block_size == 8):
                ...
                block_max_h2 = bfloat2_max_abs_8(h0, h1, h2, h3, h4, h5, h6, h7)
            block_max = bfloat2_hmax_reduce_to_f32(block_max_h2)
            local_max = cutlass.max(local_max, block_max)
            sf_idx = sf_idx + cute.arch.WARP_SIZE

        sf_idx = total_scale_blocks + tidx
        while sf_idx < allocated_scale_blocks:
            scales[sf_idx] = Uint8(0)
            sf_idx = sf_idx + cute.arch.WARP_SIZE

        amax = cute.arch.warp_reduction_max(local_max)
        if tidx == Int32(0):
            amax_tensor[Int32(0)] = amax

        global_scale = Float32(0.0)
        amax_for_error = Float32(0.0)
        if amax != Float32(0.0):
            global_scale = Float32(E2M1_MAX * E4M3_FOUROVERSIX_MAX) / amax
            amax_for_error = amax
...
class Sm120NVFP4AdaptiveSingleCtaQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
    ...
    def kernel(
        ...
        if tidx == Int32(0):
            amax_tensor[Int32(0)] = Float32(0.0)
        ...
        while sf_idx < total_scale_blocks:
            ...
            block_max = bfloat2_hmax_reduce_to_f32(block_max_h2)
            local_max = cutlass.max(local_max, block_max)
            sf_idx = sf_idx + NVFP4_BASE_THREADS_PER_BLOCK
        ...
        if tidx % cute.arch.WARP_SIZE == Int32(0):
            atomic_max_global_u32(
                get_ptr_as_int64(amax_tensor, Int32(0)),
                f32_to_u32_bits(warp_max),
            )
        cute.arch.sync_threads()
...
class Sm120ClearPaddedValues:
    ...
    def __call__(
        ...
        self.kernel(values, start_word, total_words).launch(
            grid=[num_blocks, 1, 1],
            block=[NVFP4_BASE_THREADS_PER_BLOCK, 1, 1],
            max_number_threads=[MAX_THREADS_PER_BLOCK, 1, 1],
            min_blocks_per_mp=1,
    ...
    def kernel(
        ...
        word_idx = start_word + bidx * NVFP4_BASE_THREADS_PER_BLOCK + tidx
        stride = grid_dim_x * NVFP4_BASE_THREADS_PER_BLOCK
        while word_idx < total_words:
            value_ptr = get_ptr_as_int64(values, word_idx * Int32(4))
            st_global_u32(value_ptr, Uint32(0))
            word_idx = word_idx + stride


class Sm120NVFP4AdaptiveTransposeQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
...
class Sm120NVFP4StaticQuantize2D:
    def __init__(
        self,
        k: int,
        max_quantized_value: int,
...
class Sm120NVFP4BS8StaticQuantize2D:
...
class Sm120NVFP4AdaptiveQuantize2D:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
    ...
    def kernel(
        ...
        while tile_idx < total_scale_tiles:
            ...
            dequant_scale_6 = nvfp4_compute_dequant_scale_from_amax(
                scale_fp8_u32_6,
                amax_for_error / Float32(E2M1_MAX * E4M3_FOUROVERSIX_MAX),
            )
...
class Sm120IF4AdaptiveQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
...
class Sm120IF3AdaptiveQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
    ...
    def kernel(
        ...
        while sf_idx < total_scale_blocks:
            ...
            if cutlass.const_expr(self.scale_block_size == 8):
                packed64 = (cutlass.Uint64(v1) << cutlass.Uint64(32)) | cutlass.Uint64(
                    v0
                )
                st_global_u64(output_ptr, packed64)
            else:
...
class Sm120IF4AdaptiveTransposeQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
...
class Sm120IF3AdaptiveTransposeQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
...
class Sm120IF3AdaptiveQuantize2D:
...
class Sm120IF3BS8AdaptiveQuantize2D:
    def __init__(self, k: int, scale_rule_id: int, adjustment_factor: float = 1.0):
        self.k = k
        self.scale_rule_id = scale_rule_id
        self.adjustment_factor = adjustment_factor
...
class Sm120IF6AdaptiveQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
...
class Sm120IF6AdaptivePseudoQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
    ...
    def kernel(
        ...
        while sf_idx < total_scale_blocks:
            ...
            h0, h1, h2, h3, h4, h5, h6, h7 = _process_if6_adaptive_pseudo_block_bfloat(
                x[row_idx, None],
                elem_base,
                global_scale,
                self.scale_rule_id,
...
class Sm120IF4AdaptiveQuantize2D:
...
class Sm120IF4BS8AdaptiveQuantize2D:
    def __init__(self, k: int, scale_rule_id: int):
        self.k = k
        self.scale_rule_id = scale_rule_id
...
class Sm120IF6AdaptiveQuantize2D:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
...
class Sm120MXFP4StaticQuantize:
    def __init__(
        self,
        k: int,
        max_quantized_value: int,
    ...
    def kernel(
        ...
        while sf_idx < total_scale_blocks:
            ...
            scale_ue8m0, packed64_0, packed64_1 = _process_mxfp4_static_block_bfloat(
                x[row_idx, None],
                elem_base,
                self.max_quantized_value,
                self.scale_block_size,
            ...
            if cutlass.const_expr(self.scale_block_size == 8):
                st_global_u32(
                    output_ptr0, Uint32(packed64_0 & cutlass.Uint64(0xFFFFFFFF))
                )
            else:
                output_ptr1 = get_ptr_as_int64(
                    values[row_idx, None],
...
class Sm120MXFP4StaticTransposeQuantize:
    def __init__(
        self,
        k: int,
        max_quantized_value: int,
...
class Sm120MXFP3StaticQuantize:
    def __init__(
        self,
        k: int,
...
class Sm120MXFP3StaticTransposeQuantize:
    def __init__(
        self,
        k: int,
        scale_block_size: int,
...
class Sm120MXFP3StaticQuantize2D:
...
class Sm120MXFP6StaticQuantize:
    def __init__(
        self,
        k: int,
        max_quantized_value: float,
...
class Sm120MXFP6StaticTransposeQuantize:
    def __init__(
        self,
        k: int,
        max_quantized_value: float,
...
class Sm120MXFP3BS8StaticQuantize2D:
    def __init__(self, k: int):
        self.k = k
        self.scale_blocks_per_row = k // 8
...
class Sm120MXFP4StaticQuantize2D:
    def __init__(self, k: int, max_quantized_value: int):
        self.k = k
        self.max_quantized_value = max_quantized_value
        self.scale_blocks_per_row = k // MXFP4_SCALE_BLOCK_SIZE
...
class Sm120MXFP4BS8StaticQuantize2D:
    def __init__(self, k: int, max_quantized_value: int):
        self.k = k
        self.max_quantized_value = max_quantized_value
        self.scale_blocks_per_row = k // 8
...
class Sm120MXFP6StaticQuantize2D:
...
class Sm120NVINT4StaticQuantize:
    def __init__(
        self,
        k: int,
        scale_block_size: int = NVFP4_SCALE_BLOCK_SIZE,
...
class Sm120NVINT4StaticTransposeQuantize:
    def __init__(
        self,
        k: int,
        scale_block_size: int = NVFP4_SCALE_BLOCK_SIZE,
...
class Sm120NVINT6StaticQuantize:
...
class Sm120NVINT6StaticTransposeQuantize:
    def __init__(self, k: int, adjustment_factor: float = 1.0):
        self.k = k
        self.adjustment_factor = adjustment_factor
        self.scale_blocks_per_row = k // NVFP4_SCALE_BLOCK_SIZE
...
class Sm120NVINT6StaticQuantize2D:
    def __init__(self, k: int, adjustment_factor: float = 1.0):
        self.k = k
        self.adjustment_factor = adjustment_factor
        self.scale_blocks_per_row = k // NVFP4_SCALE_BLOCK_SIZE
...
class Sm120NVINT3StaticQuantize:
    def __init__(
        self,
        k: int,
        scale_block_size: int,
...
class Sm120NVINT3StaticTransposeQuantize:
    def __init__(
        self,
        k: int,
        scale_block_size: int,
...
class Sm120NVINT3StaticQuantize2D:
    def __init__(self, k: int, adjustment_factor: float = 1.0):
        self.k = k
        self.adjustment_factor = adjustment_factor
        self.scale_blocks_per_row = k // NVFP4_SCALE_BLOCK_SIZE
...
class Sm120NVINT3BS8StaticQuantize2D:
    def __init__(self, k: int, adjustment_factor: float = 1.0):
        self.k = k
        self.adjustment_factor = adjustment_factor
        self.scale_blocks_per_row = k // 8
...
class Sm120NVFP3StaticQuantize2D:
    def __init__(self, k: int, adjustment_factor: float = 1.0):
        self.k = k
        self.adjustment_factor = adjustment_factor
        self.scale_blocks_per_row = k // NVFP4_SCALE_BLOCK_SIZE
...
class Sm120NVFP3BS8StaticQuantize2D:
    def __init__(self, k: int, adjustment_factor: float = 1.0):
        self.k = k
        self.adjustment_factor = adjustment_factor
        self.scale_blocks_per_row = k // 8
...
class Sm120NVFP6StaticQuantize:
    def __init__(
        self,
        k: int,
        max_quantized_value: float,
...
class Sm120NVFP6StaticTransposeQuantize:
    def __init__(
        self,
        k: int,
        max_quantized_value: float,
...
class Sm120NVFP3StaticQuantize:
    def __init__(
        self,
        k: int,
        scale_block_size: int,
...
class Sm120NVFP3StaticTransposeQuantize:
    def __init__(
        self,
        k: int,
        scale_block_size: int,
...
class Sm120NVINT4StaticQuantize2D:
    def __init__(self, k: int):
        self.k = k
        self.scale_blocks_per_row = k // NVFP4_SCALE_BLOCK_SIZE
...
class Sm120NVINT4BS8StaticQuantize2D:
    def __init__(self, k: int):
        self.k = k
        self.scale_blocks_per_row = k // 8
...
class Sm120NVFP6StaticQuantize2D:
    def __init__(
        self,
        k: int,
        max_quantized_value: float,
...
class Sm120NVFP4StaticPseudoQuantize:
    def __init__(
        self,
        k: int,
        max_quantized_value: int,
    ...
    def kernel(
        ...
        while sf_idx < total_scale_blocks:
            ...
            h0, h1, h2, h3, h4, h5, h6, h7 = _process_nvfp4_static_pseudo_block_bfloat(
                x[row_idx, None],
                elem_base,
                global_scale,
                self.max_quantized_value,
...
class Sm120NVFP3StaticPseudoQuantize:
    def __init__(
        self,
        k: int,
        scale_block_size: int,
    ...
    def kernel(
        ...
        while sf_idx < total_scale_blocks:
            ...
            h0, h1, h2, h3, h4, h5, h6, h7 = _process_nvfp3_static_pseudo_block_bfloat(
                x[row_idx, None],
                elem_base,
                global_scale,
                self.scale_block_size,
...
class Sm120NVFP6StaticPseudoQuantize:
    def __init__(
        self,
        k: int,
        max_quantized_value: float,
    ...
    def kernel(
        ...
        while sf_idx < total_scale_blocks:
            ...
            h0, h1, h2, h3, h4, h5, h6, h7 = _process_nvfp6_static_pseudo_block_bfloat(
                x[row_idx, None],
                elem_base,
                global_scale,
                self.max_quantized_value,
...
class Sm120NVINT6StaticPseudoQuantize:
    def __init__(self, k: int):
        self.k = k
        self.scale_blocks_per_row = k // NVFP4_SCALE_BLOCK_SIZE
    ...
    def kernel(
        ...
        while sf_idx < total_scale_blocks:
            ...
            h0, h1, h2, h3, h4, h5, h6, h7 = _process_nvint6_static_pseudo_block_bfloat(
                x[row_idx, None],
                elem_base,
                global_scale,
            )
...
class Sm120NVINT3StaticPseudoQuantize:
    ...
    def kernel(
        ...
        while sf_idx < total_scale_blocks:
            ...
            h0, h1, h2, h3, h4, h5, h6, h7 = _process_nvint3_static_pseudo_block_bfloat(
                x[row_idx, None],
                elem_base,
                global_scale,
                self.scale_block_size,
...
class Sm120NVINT4StaticPseudoQuantize:
    def __init__(self, k: int, scale_block_size: int):
        self.k = k
        self.scale_block_size = scale_block_size
        self.scale_blocks_per_row = k // scale_block_size
    ...
    def kernel(
        ...
        while sf_idx < total_scale_blocks:
            ...
            h0, h1, h2, h3, h4, h5, h6, h7 = _process_nvint4_static_pseudo_block_bfloat(
                x[row_idx, None],
                elem_base,
                global_scale,
                self.scale_block_size,
...
class Sm120MXFP4StaticPseudoQuantize:
    def __init__(
        self,
        k: int,
        max_quantized_value: int,
...
class Sm120MXFP3StaticPseudoQuantize:
    def __init__(
        self,
        k: int,
        scale_block_size: int,
...
class Sm120MXFP6StaticPseudoQuantize:
    def __init__(
        self,
        k: int,
        max_quantized_value: float,
...
class Sm120NVFP4AdaptivePseudoQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
...
class Sm120IF4AdaptivePseudoQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
    ...
    def kernel(
        ...
        while sf_idx < total_scale_blocks:
            ...
            h0, h1, h2, h3, h4, h5, h6, h7 = _process_if4_adaptive_pseudo_block_bfloat(
                x[row_idx, None],
                elem_base,
                global_scale,
                self.scale_rule_id,
...
class Sm120IF3AdaptivePseudoQuantize:
    def __init__(
        self,
        k: int,
        scale_rule_id: int,
    ...
    def kernel(
        ...
        while sf_idx < total_scale_blocks:
            ...
            h0, h1, h2, h3, h4, h5, h6, h7 = _process_if3_adaptive_pseudo_block_bfloat(
                x[row_idx, None],
                elem_base,
                global_scale,
                self.scale_rule_id,
...
class Sm120FP6DequantizeValues:
...
class Sm120INT6DequantizeValues:
    def __init__(self, k: int):
        self.k = k
        self.scale_blocks_per_row = k // NVFP4_SCALE_BLOCK_SIZE
...
class Sm120INT3DequantizeValues:
    def __init__(self, k: int):
        self.k = k
        self.scale_blocks_per_row = k // NVFP4_SCALE_BLOCK_SIZE
...
class Sm120FP3DequantizeValues:
    def __init__(self, k: int):
        self.k = k
        self.scale_blocks_per_row = k // NVFP4_SCALE_BLOCK_SIZE
...
def _compile_static_quantize(
    ...
    kernel = Sm120NVFP4StaticQuantize(
        k,
        max_quantized_value,
        scale_block_size,
        stochastic_rounding,
...
def _compile_static_transpose_quantize(
    ...
    kernel = Sm120NVFP4StaticTransposeQuantize(
        k,
        max_quantized_value,
        scale_block_size,
        stochastic_rounding,
...
def _compile_static_quantize_2d(
    ...
    kernel = Sm120NVFP4StaticQuantize2D(
        k,
        max_quantized_value,
        stochastic_rounding,
    )
...
def _compile_static_bs8_quantize_2d(k: int, max_quantized_value: int):
    ...
    kernel = Sm120NVFP4BS8StaticQuantize2D(k, max_quantized_value)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_static_pseudo_quantize(
    ...
    kernel = Sm120NVFP4StaticPseudoQuantize(
        k,
        max_quantized_value,
        scale_block_size,
        stochastic_rounding,
...
def _compile_nvfp3_static_pseudo_quantize(k: int, scale_block_size: int):
    ...
    kernel = Sm120NVFP3StaticPseudoQuantize(k, scale_block_size)
    compiled = cute.compile(
        kernel,
        x_fake,
        out_fake,
...
def _compile_nvfp6_static_pseudo_quantize(
    ...
    kernel = Sm120NVFP6StaticPseudoQuantize(k, max_quantized_value, use_e3m2)
    compiled = cute.compile(
        kernel,
        x_fake,
        out_fake,
...
def _compile_nvint6_static_pseudo_quantize(k: int):
    ...
    kernel = Sm120NVINT6StaticPseudoQuantize(k)
    compiled = cute.compile(
        kernel,
        x_fake,
        out_fake,
...
def _compile_nvint3_static_pseudo_quantize(k: int, scale_block_size: int):
    ...
    kernel = Sm120NVINT3StaticPseudoQuantize(k, scale_block_size)
    compiled = cute.compile(
        kernel,
        x_fake,
        out_fake,
...
def _compile_nvint4_static_pseudo_quantize(k: int, scale_block_size: int):
    ...
    kernel = Sm120NVINT4StaticPseudoQuantize(k, scale_block_size)
    compiled = cute.compile(
        kernel,
        x_fake,
        out_fake,
...
def _compile_mxfp4_static_pseudo_quantize(
    ...
    kernel = Sm120MXFP4StaticPseudoQuantize(
        k,
        max_quantized_value,
        scale_block_size,
    )
...
def _compile_mxfp3_static_pseudo_quantize(k: int, scale_block_size: int):
    ...
    kernel = Sm120MXFP3StaticPseudoQuantize(k, scale_block_size)
    compiled = cute.compile(
        kernel,
        x_fake,
        out_fake,
...
def _compile_mxfp6_static_pseudo_quantize(
    ...
    kernel = Sm120MXFP6StaticPseudoQuantize(k, max_quantized_value, use_e3m2)
    compiled = cute.compile(
        kernel,
        x_fake,
        out_fake,
...
def _compile_adaptive_quantize(
    ...
    kernel = Sm120NVFP4AdaptiveQuantize(
        k,
        scale_rule_id,
        stochastic_rounding,
        scale_block_size,
...
@functools.cache
def _compile_adaptive_small_m_quantize(
    k: int,
    scale_rule_id: int,
    stochastic_rounding: bool = False,
    scale_block_size: int = NVFP4_SCALE_BLOCK_SIZE,
    ...
    kernel = Sm120NVFP4AdaptiveSmallMQuantize(
        k,
        scale_rule_id,
        stochastic_rounding,
        scale_block_size,
...
@functools.cache
def _compile_adaptive_single_cta_quantize(
    k: int,
    scale_rule_id: int,
    stochastic_rounding: bool = False,
    scale_block_size: int = NVFP4_SCALE_BLOCK_SIZE,
    ...
    kernel = Sm120NVFP4AdaptiveSingleCtaQuantize(
        k,
        scale_rule_id,
        stochastic_rounding,
        scale_block_size,
...
@functools.cache
def _compile_clear_padded_values():
    sym_values = cute.sym_int()
    values_fake = cute.runtime.make_fake_compact_tensor(
        cutlass.Uint8,
        (sym_values,),
        assumed_align=16,
...
def _compile_adaptive_transpose_quantize(
    ...
    kernel = Sm120NVFP4AdaptiveTransposeQuantize(k, scale_rule_id)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_adaptive_quantize_2d(
    ...
    kernel = Sm120NVFP4AdaptiveQuantize2D(
        k,
        scale_rule_id,
        match_triton_reduction,
    )
...
def _compile_if3_adaptive_quantize(
    ...
    kernel = Sm120IF3AdaptiveQuantize(
        k,
        scale_rule_id,
        scale_block_size,
        adjustment_factor,
...
def _compile_if3_adaptive_transpose_quantize(
    ...
    kernel = Sm120IF3AdaptiveTransposeQuantize(
        k,
        scale_rule_id,
        scale_block_size,
        adjustment_factor,
...
def _compile_if4_adaptive_quantize(
    ...
    kernel = Sm120IF4AdaptiveQuantize(
        k,
        scale_rule_id,
        stochastic_rounding,
        scale_block_size,
...
def _compile_if4_adaptive_transpose_quantize(
    ...
    kernel = Sm120IF4AdaptiveTransposeQuantize(
        k,
        scale_rule_id,
        scale_block_size,
    )
...
def _compile_if3_adaptive_quantize_2d(
    ...
    kernel = Sm120IF3AdaptiveQuantize2D(k, scale_rule_id, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_if3_bs8_adaptive_quantize_2d(
    ...
    kernel = Sm120IF3BS8AdaptiveQuantize2D(k, scale_rule_id, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_if6_adaptive_quantize(
    ...
    kernel = Sm120IF6AdaptiveQuantize(
        k,
        scale_rule_id,
        max_quantized_value,
...
def _compile_if4_adaptive_quantize_2d(k: int, scale_rule_id: int):
    ...
    kernel = Sm120IF4AdaptiveQuantize2D(k, scale_rule_id)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_if4_bs8_adaptive_quantize_2d(k: int, scale_rule_id: int):
    ...
    kernel = Sm120IF4BS8AdaptiveQuantize2D(k, scale_rule_id)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_if6_adaptive_pseudo_quantize(
    ...
    kernel = Sm120IF6AdaptivePseudoQuantize(
        k,
        scale_rule_id,
        max_quantized_value,
        int_expansion_factor,
...
def _compile_if6_adaptive_quantize_2d(
    ...
    kernel = Sm120IF6AdaptiveQuantize2D(
        k,
        scale_rule_id,
        max_quantized_value,
        int_expansion_factor,
...
def _compile_mxfp4_static_quantize(
    ...
    kernel = Sm120MXFP4StaticQuantize(
        k,
        max_quantized_value,
        scale_block_size,
        stochastic_rounding,
...
def _compile_mxfp4_static_transpose_quantize(
    ...
    kernel = Sm120MXFP4StaticTransposeQuantize(
        k,
        max_quantized_value,
        scale_block_size,
        stochastic_rounding,
...
def _compile_mxfp3_static_quantize(
    ...
    kernel = Sm120MXFP3StaticQuantize(k, scale_block_size)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_mxfp3_static_transpose_quantize(
    ...
    kernel = Sm120MXFP3StaticTransposeQuantize(k, scale_block_size)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_mxfp3_static_quantize_2d(k: int):
    ...
    kernel = Sm120MXFP3StaticQuantize2D(k)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_mxfp3_bs8_static_quantize_2d(k: int):
    ...
    kernel = Sm120MXFP3BS8StaticQuantize2D(k)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_mxfp6_static_quantize(
    ...
    kernel = Sm120MXFP6StaticQuantize(k, max_quantized_value, use_e3m2)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_mxfp6_static_transpose_quantize(
    ...
    kernel = Sm120MXFP6StaticTransposeQuantize(k, max_quantized_value, use_e3m2)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_mxfp6_static_quantize_2d(
    ...
    kernel = Sm120MXFP6StaticQuantize2D(k, max_quantized_value, use_e3m2)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_mxfp4_static_quantize_2d(k: int, max_quantized_value: int):
    ...
    kernel = Sm120MXFP4StaticQuantize2D(k, max_quantized_value)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_mxfp4_bs8_static_quantize_2d(k: int, max_quantized_value: int):
    ...
    kernel = Sm120MXFP4BS8StaticQuantize2D(k, max_quantized_value)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvint4_static_quantize(
    ...
    kernel = Sm120NVINT4StaticQuantize(
        k,
        scale_block_size,
        stochastic_rounding,
        adjustment_factor,
...
def _compile_nvint4_static_transpose_quantize(
    ...
    kernel = Sm120NVINT4StaticTransposeQuantize(
        k,
        scale_block_size,
        stochastic_rounding,
        adjustment_factor,
...
def _compile_nvfp3_static_quantize(
    ...
    kernel = Sm120NVFP3StaticQuantize(k, scale_block_size, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvfp3_static_transpose_quantize(
    ...
    kernel = Sm120NVFP3StaticTransposeQuantize(k, scale_block_size, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvint3_static_quantize(
    ...
    kernel = Sm120NVINT3StaticQuantize(k, scale_block_size, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvint3_static_transpose_quantize(
    ...
    kernel = Sm120NVINT3StaticTransposeQuantize(k, scale_block_size, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvint3_static_quantize_2d(
    ...
    kernel = Sm120NVINT3StaticQuantize2D(k, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvint3_bs8_static_quantize_2d(
    ...
    kernel = Sm120NVINT3BS8StaticQuantize2D(k, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvfp3_static_quantize_2d(
    ...
    kernel = Sm120NVFP3StaticQuantize2D(k, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvfp3_bs8_static_quantize_2d(
    ...
    kernel = Sm120NVFP3BS8StaticQuantize2D(k, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvint6_static_quantize(
    ...
    kernel = Sm120NVINT6StaticQuantize(k, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvint6_static_transpose_quantize(
    ...
    kernel = Sm120NVINT6StaticTransposeQuantize(k, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvint6_static_quantize_2d(
    ...
    kernel = Sm120NVINT6StaticQuantize2D(k, adjustment_factor)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvint4_static_quantize_2d(k: int):
    ...
    kernel = Sm120NVINT4StaticQuantize2D(k)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvint4_bs8_static_quantize_2d(k: int):
    ...
    kernel = Sm120NVINT4BS8StaticQuantize2D(k)
    compiled = cute.compile(
        kernel,
        x_fake,
        values_fake,
...
def _compile_nvfp6_static_quantize(
    ...
    kernel = Sm120NVFP6StaticQuantize(
        k,
        max_quantized_value,
        use_e3m2,
        adjustment_factor,
...
def _compile_nvfp6_static_transpose_quantize(
    ...
    kernel = Sm120NVFP6StaticTransposeQuantize(
        k,
        max_quantized_value,
        use_e3m2,
        adjustment_factor,
...
def _compile_nvfp6_static_quantize_2d(
    ...
    kernel = Sm120NVFP6StaticQuantize2D(
        k,
        max_quantized_value,
        use_e3m2,
        adjustment_factor,
...
def _compile_adaptive_pseudo_quantize(
    ...
    kernel = Sm120NVFP4AdaptivePseudoQuantize(
        k,
        scale_rule_id,
        stochastic_rounding,
        scale_block_size,
...
def _compile_if4_adaptive_pseudo_quantize(
    ...
    kernel = Sm120IF4AdaptivePseudoQuantize(k, scale_rule_id, scale_block_size)
    compiled = cute.compile(
        kernel,
        x_fake,
        out_fake,
...
def _compile_if3_adaptive_pseudo_quantize(
    ...
    kernel = Sm120IF3AdaptivePseudoQuantize(k, scale_rule_id, scale_block_size)
    compiled = cute.compile(
        kernel,
        x_fake,
        out_fake,
...
def _compile_fp6_dequantize_values(k: int, use_e3m2: bool, is_if6: bool):
    ...
    kernel = Sm120FP6DequantizeValues(k, use_e3m2, is_if6)
    compiled = cute.compile(
        kernel,
        values_fake,
        scales_fake,
...
def _compile_int3_dequantize_values(k: int):
    ...
    kernel = Sm120INT3DequantizeValues(k)
    compiled = cute.compile(
        kernel,
        values_fake,
        out_fake,
...
def _compile_fp3_dequantize_values(k: int):
    ...
    kernel = Sm120FP3DequantizeValues(k)
    compiled = cute.compile(
        kernel,
        values_fake,
        out_fake,
...
def _compile_int6_dequantize_values(k: int):
    ...
    kernel = Sm120INT6DequantizeValues(k)
    compiled = cute.compile(
        kernel,
        values_fake,
        out_fake,
...
def _validate_quantize_input(
    ...
    if x.device.type != "cuda":
        msg = "CuTe sm120 NVFP4 quantize requires a CUDA tensor"
        raise ValueError(msg)
    if x.dtype != torch.bfloat16:
        msg = f"CuTe sm120 NVFP4 quantize requires bfloat16 input, got {x.dtype}"
        raise ValueError(msg)
    if x.ndim != 2:
        msg = f"CuTe sm120 NVFP4 quantize requires a 2D tensor, got {x.ndim}D"
        raise ValueError(msg)
...
def _validate_mxfp4_input(
    ...
    if x.device.type != "cuda":
        msg = "CuTe sm120 MXFP4 quantize requires a CUDA tensor"
        raise ValueError(msg)
    if x.dtype != torch.bfloat16:
        msg = f"CuTe sm120 MXFP4 quantize requires bfloat16 input, got {x.dtype}"
        raise ValueError(msg)
    if x.ndim != 2:
        msg = f"CuTe sm120 MXFP4 quantize requires a 2D tensor, got {x.ndim}D"
        raise ValueError(msg)
    ...
    if x.shape[1] % scale_block_size != 0:
        msg = (
            f"last dimension must be divisible by {scale_block_size}, got {x.shape[1]}"
        )
        raise ValueError(msg)
    if max_quantized_value not in {4, 6}:
...
def _adaptive_min_blocks_per_mp(m: int, k: int) -> int:
    if m < 512:
        return 10
...
def _clear_padded_values(
    values: torch.Tensor,
    m: int,
    k: int,
) -> None:
    padded_m = values.shape[0]
    if padded_m != m:
        values[m:padded_m].zero_()
...
def quantize_nvfp4_static_2d(
    ...
    *,
    ...
    total_scale_tiles = (m // NVFP4_SCALE_BLOCK_SIZE) * (k // NVFP4_SCALE_BLOCK_SIZE)
...
def quantize_mxfp3_static_2d(
    ...
    total_scale_tiles = (m // MXFP4_SCALE_BLOCK_SIZE) * (k // MXFP4_SCALE_BLOCK_SIZE)
...
def quantize_nvfp4_adaptive(
    ...
    *,
    ...
    scale_factors_dtype: torch.dtype = torch.uint8,
    clear_padded_scales: bool = True,
    clear_padded_values: bool = True,
    match_triton_reduction: bool = False,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    if k % scale_block_size != 0:
        ...
        raise ValueError(msg)
    padded_m = (
        m if pad_rows_to is None else m + (pad_rows_to - m % pad_rows_to) % pad_rows_to
    )
    x = x.contiguous()

    values = torch.empty((padded_m, k // 2), dtype=torch.uint8, device=x.device)
    scale_factors = torch.empty(
        (padded_m, k // scale_block_size),
        dtype=scale_factors_dtype,
        device=x.device,
    )
    if x_amax is None and m <= 6:
        amax = torch.empty((1,), dtype=torch.float32, device=x.device)
        kernel = _compile_adaptive_small_m_quantize(
            k,
            scale_rule_id,
            stochastic_rounding,
            scale_block_size,
        ...
        kernel(
            ...
            (
                ...
                else m * (k // scale_block_size)
            ),
            amax,
            cutlass_torch.current_stream(),
        )
        if clear_padded_values:
            _clear_padded_values(values, m, k)
        return values, scale_factors, amax.reshape(())
    total_scale_blocks = m * (k // scale_block_size)
    if x_amax is None and m <= 256:
        amax = torch.empty((1,), dtype=torch.float32, device=x.device)
        kernel = _compile_adaptive_single_cta_quantize(
            k,
            scale_rule_id,
            stochastic_rounding,
            scale_block_size,
    ...
    kernel = _compile_adaptive_quantize(
        ...
        stochastic_rounding,
        scale_block_size,
        _adaptive_min_blocks_per_mp(m, k),
        match_triton_reduction,
    )
    kernel(
        x,
...
def quantize_nvfp4_adaptive_into(
    x: torch.Tensor,
    values: torch.Tensor,
    ...
    *,
    ...
    total_scale_blocks = m * (k // scale_block_size)
    allocated_scale_blocks = (
        scale_factors.numel() if clear_padded_scales else total_scale_blocks
    )
    num_blocks = _uncapped_launch_grid(
        total_scale_blocks,
        threads_per_block=NVFP4_BASE_THREADS_PER_BLOCK,
...
def quantize_nvfp4_adaptive_into_unchecked(
    x: torch.Tensor,
    values: torch.Tensor,
    scale_factors: torch.Tensor,
    amax: torch.Tensor,
...
def quantize_if4_adaptive(
    ...
    *,
    ...
    if scale_block_size == 8 and stochastic_rounding:
        msg = "IF4_BS8 stochastic rounding is not supported by the CuTe sm120 backend"
        raise ValueError(msg)
    m, k = _validate_quantize_input(x, 6)
    if k % scale_block_size != 0:
        msg = f"columns must be divisible by {scale_block_size}, got {k}"
